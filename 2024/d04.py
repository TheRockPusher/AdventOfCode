...
def search(
    ...
    if (not 0 <= nextCoord[0] < len(lineFile[0])) or (
        not 0 <= nextCoord[1] < len(lineFile)
    ):
        return False
...
def searchAllDirections(lineFile, coord: tuple[int, int], check_letter: int, word: str):
    ...
    return sum(
        [
            horizontal_search,
            horizontal_opp_search,
            vertical_search,
            vertical_opp_search,
            diagonal_search,
            diagonal_opp_search,
            diagonal_back_search,
            diagonal_back_opp_search,
        ]
    )
...
# Exercise 2
def foundX(lineFile, coord: tuple[int, int], check_letter: int, word: str):
    startCoordX1 = (coord[0] - 2, coord[1] - 2)
    startCoordX2 = (coord[0] + 2, coord[1] - 2)
    diagonal = search(lineFile, startCoordX1, (1, 1), 0, word) or search(
        lineFile, startCoordX1, (1, 1), 0, word[::-1]
    )
    diagonal2 = search(lineFile, startCoordX2, (-1, 1), 0, word) or search(
        lineFile, startCoordX2, (-1, 1), 0, word[::-1]
    )
    return diagonal and diagonal2
...
for y, line in enumerate(lineFile):
    for x, letter in enumerate(line):
        if letter == "A":
            totalEx2 += foundX(lineFile, (x, y), 1, "MAS")
print(totalEx2)
